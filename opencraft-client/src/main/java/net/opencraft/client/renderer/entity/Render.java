...
import net.opencraft.client.renderer.entity.models.ModelBase;
...
public abstract class Render<T extends Entity> {
    ...
    private void renderEntityOnFire(final Entity eq, final double double2, final double double3, final double double4, final float float5) {
        ...
        instance.beginQuads();
        while (n11 > 0.0f) {
            instance.vertexUV(n8 - n9, 0.0f - n10, 0.0, n4, n6);
            instance.vertexUV(0.0f - n9, 0.0f - n10, 0.0, n3, n6);
            instance.vertexUV(0.0f - n9, 1.4f - n10, 0.0, n3, n5);
            instance.vertexUV(n8 - n9, 1.4f - n10, 0.0, n4, n5);
            --n11;
            --n10;
            n8 *= 0.9f;
    ...
    private void renderShadow(final Entity eq, final double double2, final double double3, final double double4, final float float5, final float float6) {
        ...
        final ClientWorld clientWorldFromRenderManager = this.getWorldFromRenderManager();
        ...
        for (int i = floor_double; i <= floor_double2; ++i) {
            for (int j = floor_double3; j <= floor_double4; ++j) {
                for (int k = floor_double5; k <= floor_double6; ++k) {
                    final int blockId = clientWorldFromRenderManager.getBlockId(i, j - 1, k);
                    if (blockId > 0 && clientWorldFromRenderManager.getBlockLightValue(i, j, k) > 3) {
    ...
    private ClientWorld getWorldFromRenderManager() {
        return this.renderManager.clientWorldObj;
    }
