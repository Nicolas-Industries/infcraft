...
public class GuiChest extends GuiContainer {
    ...
    public GuiChest(final IInventory kd1, final IInventory kd2) {
        ...
        for (int i = 0; i < this.inventoryRows; ++i) {
            for (int j = 0; j < 9; ++j) {
                this.inventorySlots.add(new Slot(kd2, j + i * 9, 8 + j * 18, 18 + i * 18));
            }
        }
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 9; ++j) {
                this.inventorySlots.add(new Slot(kd1, j + (i + 1) * 9, 8 + j * 18, 103 + i * 18 + n2));
            }
        }
        for (int i = 0; i < 9; ++i) {
            this.inventorySlots.add(new Slot(kd1, i, 8 + i * 18, 161 + n2));
        }
    }
