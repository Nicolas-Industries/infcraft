...
import net.opencraft.core.inventory.InventoryCraftResult;
import net.opencraft.core.inventory.InventoryCrafting;
import net.opencraft.core.inventory.InventoryPlayer;
import net.opencraft.core.inventory.Slot;
import net.opencraft.core.inventory.SlotCrafting;
...
public class GuiCrafting extends GuiContainer {
    ...
    public GuiCrafting(final InventoryPlayer ht) {
        this.inventoryCrafting = new InventoryCrafting( 3, 3);
        this.iInventory = new InventoryCraftResult();
        this.inventorySlots.add(new SlotCrafting(this.inventoryCrafting, this.iInventory, 0, 124, 35));
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
                this.inventorySlots.add(new Slot(this.inventoryCrafting, j + i * 3, 30 + j * 18, 17 + i * 18));
            }
        }
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 9; ++j) {
                this.inventorySlots.add(new Slot(ht, j + (i + 1) * 9, 8 + j * 18, 84 + i * 18));
            }
        }
        for (int i = 0; i < 9; ++i) {
            this.inventorySlots.add(new Slot(ht, i, 8 + i * 18, 142));
        }
    }
