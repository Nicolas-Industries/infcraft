...
public class Chunk {
    ...
    public Chunk(final ClientWorld world, final int xCoord, final int zCoord) {
    ...
    public Chunk(final ClientWorld world, final byte[] byteArray, final int xCoord, final int zCoord) {
    ...
    private void relightBlock(final int xCoord, final int yCoord, final int zCoord) {
        ...
        this.clientWorldObj.markBlocksDirtyVertical(xCoord, zCoord, n2, n);
        ...
        if (n2 < n) {
            ...
            this.clientWorldObj.scheduleLightingUpdate(EnumSkyBlock.Sky, n3, n, i, n3, n2, i);
            for (int j = n; j < n2; ++j) {
        ...
        if (n2 != integer6) {
            this.clientWorldObj.scheduleLightingUpdate(EnumSkyBlock.Sky, n3 - 1, n2, i - 1, n3 + 1, integer6, i + 1);
        }
        this.isModified = true;
    }
    ...
    public void onChunkLoad() {
        ...
        this.clientWorldObj.loadedTileEntityList.addAll(this.chunkTileEntityMap.values());
        for (int i = 0; i < this.entities.length; ++i) {
            this.clientWorldObj.addLoadedEntities(this.entities[i]);
        }
    }

    public void onChunkUnload() {
        this.isChunkLoaded = false;
        this.clientWorldObj.loadedTileEntityList.removeAll(this.chunkTileEntityMap.values());
        for (int i = 0; i < this.entities.length; ++i) {
            this.clientWorldObj.unloadEntities(this.entities[i]);
        }
    }
