...
public class Pathfinder {
	...
	public PathEntity createEntityPathTo(final Entity eq, final int integer2, final int integer3, final int integer4,
			final float float5) {
		return this.createEntityPathTo(eq, integer2 + 0.5f, integer3 + 0.5f, integer4 + 0.5f, float5);
	}

	private PathEntity createEntityPathTo(final Entity eq, final double double2, final double double3,
			final double double4, final float float5) {
		this.path.clearPath();
		this.pointMap.clear();
		return this.addToPath(eq,
				this.openPoint(Mth.floor_double(eq.boundingBox.minX), Mth.floor_double(eq.boundingBox.minY),
						Mth.floor_double(eq.boundingBox.minZ)),
				this.openPoint(Mth.floor_double(double2 - eq.width / 2.0f), Mth.floor_double(double3),
						Mth.floor_double(double4 - eq.width / 2.0f)),
				new PathPoint((int) floor(eq.width + 1.0f), (int) floor(eq.height + 1.0f),
						(int) floor(eq.width + 1.0f)),
				float5);
	}

	private PathEntity addToPath(final Entity eq, final PathPoint d2, final PathPoint d3, final PathPoint d4,
			final float float5) {
	...
	private int findPathOptions(final Entity eq, final PathPoint d2, final PathPoint d3, final PathPoint d4,
			final float float5) {
		int n = 0;
		int n2 = 0;
	...
	private PathPoint getSafePoint(final Entity eq, final int integer2, int integer3, final int integer4,
			final PathPoint d, final int integer6) {
		...
		if (pathPoint != null) {
			...
			int verticalOffset;
			while (integer3 > 0
					&& (verticalOffset = this.getVerticalOffset(eq, integer2, integer3 - 1, integer4, d)) > 0) {
				if (verticalOffset < 0) {
					return null;
				}
				if (++n >= 4) {
	...
	private int getVerticalOffset(final Entity eq, final int integer2, final int integer3, final int integer4,
			final PathPoint d) {
