...
import net.infcraft.core.entity.*;
...
import net.infcraft.client.world.IWorldAccess;
import org.lwjgl.BufferUtils;
import org.lwjgl.opengl.ARBOcclusionQuery;
import org.lwjgl.opengl.GL11;

import net.infcraft.client.InfCraft;
import net.infcraft.client.fi;
...
import net.infcraft.client.renderer.culling.ICamera;
import net.infcraft.client.renderer.tileentity.TileEntityRenderer;
...
public class RenderGlobal implements IWorldAccess {
	...
	public List a;
	private ClientWorld clientWorldObj;
	private Renderer l;
	private List m;
	...
	public RenderGlobal(final InfCraft aw, final Renderer id) {
	...
	public void renderEntities(final Vec3 bo, final ICamera jt, final float float3) {
		TileEntityRenderer.instance.a(this.clientWorldObj, this.l, this.t.font, this.t.player, float3);
		RenderManager.instance.cacheActiveRenderInfo(this.clientWorldObj, this.l, this.t.font, this.t.player,
		...
		final List loadedEntityList = this.clientWorldObj.getLoadedEntityList();
		...
		for (int i = 0; i < loadedEntityList.size(); ++i) {
			...
			if (eq.isInRangeToRenderVec3D(bo) && jt.isBoundingBoxInFrustum(eq.boundingBox)) {
				if (eq != this.clientWorldObj.player || this.t.options.thirdPersonView) {
					++this.J;
	...
	public int sortAndRender(final EntityPlayerLocal gi, final int integer, final double double3) {
	...
	private int a(final int integer1, final int integer2, final int integer3, final double double4) {
		...
		final EntityPlayerLocal thePlayer = this.t.player;
	...
	public void renderSky(final float float1) {
		GL11.glDisable(3553);
		final Vec3 skyColor = this.clientWorldObj.getSkyColor(float1);
		...
		GL11.glRotatef(this.clientWorldObj.getCelestialAngle(float1) * 360.0f, 1.0f, 0.0f, 0.0f);
		float n8 = 30.0f;
		...
		instance.draw();
		GL11.glDisable(3553);
		final float starBrightness = this.clientWorldObj.getStarBrightness(float1);
	...
	public void renderClouds(final float float1) {
		...
		final Vec3 drawClouds = this.clientWorldObj.drawClouds(float1);
	...
	public boolean updateRenderers(final EntityPlayerLocal gi, final boolean boolean2) {
	...
	public void drawBlockBreaking(final EntityPlayerLocal gi, final MovingObjectPosition hb, final int integer,
		...
		if (integer == 0) {
			if (this.damagePartialTime > 0.0f) {
				...
				GL11.glPushMatrix();
				final int n = this.clientWorldObj.getBlockId(hb.blockX, hb.blockY, hb.blockZ);
	...
	public void drawSelectionBox(final EntityPlayerLocal gi, final MovingObjectPosition hb, final int integer,
                                 final ItemStack hw, final float float5) {
		if (integer == 0 && hb.typeOfHit == 0) {
			...
			final int blockId = this.clientWorldObj.getBlockId(hb.blockX, hb.blockY, hb.blockZ);
			if (blockId > 0) {
				this.a(Block.blocksList[blockId].getSelectedBoundingBoxFromPool(null, hb.blockX, hb.blockY, hb.blockZ)
