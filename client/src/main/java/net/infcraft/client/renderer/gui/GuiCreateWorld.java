...
public class GuiCreateWorld extends GuiScreen {
    ...
    public void initGui() {
        ...
        final File gameDir = InfCraft.getGameDir();
        final String[] worldNames = ClientWorld.getSaveNames(gameDir);
        final int worldCount = worldNames.length;

        for (int i = 0; i < worldCount; ++i) {
            final NBTTagCompound potentiallySavesFolderLocation = ClientWorld.potentiallySavesFolderLocation(gameDir, worldNames[i]);
    ...
    protected String getSaveFileName(final int integer) {
        return (ClientWorld.potentiallySavesFolderLocation(InfCraft.getGameDir(), new StringBuilder().append("World").append(integer).toString()) != null) ? new StringBuilder().append("World").append(integer).toString() : null;
    }
    ...
    protected void actionPerformed(final GuiButton iq) {
        ...
        if (iq.buttonId == -20) {
            final String[] worldNames = ClientWorld.getSaveNames(InfCraft.getGameDir());
