...
public class SoundManager {
    ...
    public static final String SOUNDS_PATH = "assets/resources/";
    ...
    public boolean containsSound(String soundName) {
        return soundPoolSounds.contains(soundName) || soundPoolStreaming.contains(soundName)
                || soundPoolIngameMusic.contains(soundName) || soundPoolMenuMusic.contains(soundName);
    }
    ...
    public void registerSound(final URL resourceURL) {
        String name = resourceURL.getPath()
                .substring(resourceURL.getPath().lastIndexOf(SOUNDS_PATH) + SOUNDS_PATH.length());
        // Don't strip numbers yet, keep unique name
        name = name.substring(name.indexOf("/") + 1).replace("%20", " ").replace("/", ".");
        final String path = resourceURL.getPath();

        if (!switch (SoundType.fromString(path)) {
            case MENUMUSIC -> addMenuMusic(name, resourceURL);
            case SOUND -> {
                // For sound effects, we want to group variations (e.g. stone1, stone2) under a
                // common key (stone)
                // But we must keep the unique name for the SoundSystem to cache them separately
                String key = name.replaceAll("[0-9]", "");
                if (key.contains(".")) {
                    key = key.substring(0, key.lastIndexOf("."));
                }
                yield addSound(key, name, resourceURL);
            }
    ...
    public boolean addSound(String key, String name, URL resourceURL) {
        return soundPoolSounds.addSound(key, name, resourceURL) != null;
    }
    ...
    public void stopSound(String var1) {
        if (sndSystem == null)
            return; // TODO: Temporary fix to InfCraft problem
        ...
        if (this.loaded && this.options.music) {
            ...
            if (var1 != null) {
                ...
                if (var8 != null && var5 > 0.0F) {
                    ...
                    float var9 = 16.0F;
                    this.sndSystem.newStreamingSource(true, var7, var8.soundUrl, var8.soundName, false, var2, var3,
                            var4, 2, var9 * 4.0F);
    ...
    public void playSound(String var1, float var2, float var3, float var4, float var5, float var6) {
        if (this.loaded && this.options.sound) {
            ...
            if (var7 != null && var5 > 0.0F) {
                ...
                this.sndSystem.newSource(var5 > 1.0F, var8, var7.soundUrl, var7.soundName, false, var2, var3, var4, 2,
                        var9);
