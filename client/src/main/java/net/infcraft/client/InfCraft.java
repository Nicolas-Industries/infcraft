...
import net.infcraft.client.entity.PlayerController;
import net.infcraft.client.input.KeyboardInput;
import net.infcraft.client.input.MouseInput;
import net.infcraft.client.input.MovementInput;
import net.infcraft.client.network.ClientNetworkManager;
import net.infcraft.client.renderer.EffectRenderer;
import net.infcraft.client.renderer.GLAllocation;
import net.infcraft.client.renderer.LoadingScreenRenderer;
import net.infcraft.client.renderer.Tessellator;
import net.infcraft.client.renderer.gui.*;
import net.infcraft.client.renderer.texture.*;
import net.infcraft.client.resources.DownloadResourcesJob;
...
import net.infcraft.client.renderer.entity.models.ModelBiped;
import net.infcraft.client.sound.SoundManager;
import net.infcraft.client.entity.EntityPlayerLocal;
import net.infcraft.client.renderer.entity.EntityRenderer;
import net.infcraft.core.physics.AABB;
import net.infcraft.client.renderer.entity.RenderGlobal;
import net.infcraft.client.renderer.entity.Renderer;
import net.infcraft.client.renderer.font.FontRenderer;
import net.infcraft.core.util.*;
...
public class InfCraft implements Runnable {
	...
	static {
		...
		InfCraft.numRecordedFrameTimes = 0;
	...
	public InfCraft(final int width, final int height, final boolean boolean6) {
		oc = this;
		...
		try {
			...
			while (running) {
				...
				if (!skipRenderWorld) {
					...
					renderGlobal.damagePartialTime = playerController.getCurBlockDamageMP();
	...
	private void displayDebugInfo() {
		...
		InfCraft.tickTimes[InfCraft.numRecordedFrameTimes++ & InfCraft.tickTimes.length - 1] = nanoTime
				- prevFrameTime;
		...
		instance.vertex(InfCraft.tickTimes.length, height, 0.0);
		instance.vertex(InfCraft.tickTimes.length, height - 100, 0.0);
		instance.draw();
		long n = 0L;
		for (int i = 0; i < InfCraft.tickTimes.length; ++i) {
			n += InfCraft.tickTimes[i];
		}
		int i = (int) (n / 200000L / InfCraft.tickTimes.length);
		instance.begin(7);
		instance.setColorOpaque_I(541065216);
		instance.vertex(0.0, height - i, 0.0);
		instance.vertex(0.0, height, 0.0);
		instance.vertex(InfCraft.tickTimes.length, height, 0.0);
		instance.vertex(InfCraft.tickTimes.length, height - i, 0.0);
		instance.draw();
		instance.begin(1);
		for (int j = 0; j < InfCraft.tickTimes.length; ++j) {
			final int n2 = (j - InfCraft.numRecordedFrameTimes & InfCraft.tickTimes.length - 1) * 255
					/ InfCraft.tickTimes.length;
			int n3 = n2 * n2 / 255;
			n3 = n3 * n3 / 255;
			int n4 = n3 * n3 / 255;
			n4 = n4 * n4 / 255;
			instance.setColorOpaque_I(-16777216 + n4 + n3 * 256 + n2 * 65536);
			instance.vertex(j + 0.5f, height - InfCraft.tickTimes[j] / 200000L + 0.5f, 0.0);
	...
	private void clickMouse(final int integer) {
		...
		if (objectMouseOver == null) {
			...
			if (integer == 0) {
				...
				// Don't call clickBlock here - it resets damage and interferes with progressive
				// breaking
				// Block breaking is handled by sendBlockRemoving which is called continuously
			} else {
				...
				net.infcraft.shared.network.packets.PacketBlockPlacement packet = new net.infcraft.shared.network.packets.PacketBlockPlacement(
				...
				if (currentItem != null && currentItem.getItem() instanceof net.infcraft.core.item.ItemBlock) {
					entityRenderer.itemRenderer.resetEquippedProgress();
				}
	...
	private void clickMiddleMouseButton() {
		if (objectMouseOver != null) {
			...
			sendHeldItemChange(player.inventory.currentItem);
		}
	}
	...
	public void changeWorld2(final ClientWorld fe, final String string) {
		...
		// FIX: Clear player when closing world to ensure fresh state for next world
		if (fe == null) {
			player = null;
		}
		...
		if ((clientWorld = fe) != null) {
			...
			if (player == null) {
				// Create the player
				(player = new EntityPlayerLocal(oc, fe, sessionData)).preparePlayerToSpawn();
				if (isMultiplayerWorld()) {
					getClientNetworkManager().getLocalNetworkManager().setPlayer(player);
				}
	...
	public void respawn() {
		...
		clientWorld.a();
		(player = new EntityPlayerLocal(oc, clientWorld, sessionData)).preparePlayerToSpawn();
		playerController.flipPlayer(player);
		if (clientWorld != null) {
	...
	private void sendHeldItemChange(int slot) {
		if (this.getClientNetworkManager() != null) {
			try {
				this.getClientNetworkManager()
						.sendPacket(new net.infcraft.shared.network.packets.PacketHeldItemChange(slot));
			} catch (java.io.IOException e) {
				e.printStackTrace();
			}
		}
