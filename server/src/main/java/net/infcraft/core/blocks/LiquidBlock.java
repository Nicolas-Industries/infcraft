...
public abstract class LiquidBlock extends Block {
    ...
    protected int getFlowDecay(final ServerWorld serverWorld, final int xCoord, final int yCoord, final int zCoord) {
        if (serverWorld.getBlockMaterial(xCoord, yCoord, zCoord) != this.blockMaterial) {
            return -1;
        }
        return serverWorld.getBlockMetadata(xCoord, yCoord, zCoord);
    }

    protected int getEffectiveFlowDecay(final IBlockAccess blockAccess, final int xCoord, final int yCoord,
            final int zCoord) {
    ...
    public boolean shouldSideBeRendered(final IBlockAccess blockAccess, final int xCoord, final int yCoord,
        ...
        return blockAccess.getBlockMaterial(xCoord, yCoord, zCoord) != this.blockMaterial
                && (nya4 == 1 || super.shouldSideBeRendered(blockAccess, xCoord, yCoord, zCoord, nya4));
    }
    ...
    @Override
    public void velocityToAddToEntity(final World world, final int xCoord, final int yCoord, final int zCoord,
            final Entity entity, final Vec3 var1) {
    ...
    public void randomDisplayTick(final ServerWorld serverWorld, final int xCoord, final int yCoord, final int zCoord,
        ...
        if (this.blockMaterial == Material.WATER && random.nextInt(64) == 0) {
            ...
            if (blockMetadata > 0 && blockMetadata < 8) {
                serverWorld.playSoundEffect((xCoord + 0.5f), (yCoord + 0.5f), (zCoord + 0.5f), "liquid.water",
                        random.nextFloat() * 0.25f + 0.75f, random.nextFloat() * 1.0f + 0.5f);
            }
        }
        if (this.blockMaterial == Material.LAVA
                && serverWorld.getBlockMaterial(xCoord, yCoord + 1, zCoord) == Material.AIR
                && !serverWorld.isBlockNormalCube(xCoord, yCoord + 1, zCoord) && random.nextInt(100) == 0) {
            serverWorld.spawnParticle("lava", (xCoord + random.nextFloat()), yCoord + this.maxY,
                    (zCoord + random.nextFloat()), 0.0, 0.0, 0.0);
        }
    }

    @Override
    public void onBlockAdded(final ServerWorld serverWorld, final int xCoord, final int yCoord, final int zCoord) {
        this.checkForHarden(serverWorld, xCoord, yCoord, zCoord);
    }
    ...
    private void checkForHarden(final ServerWorld serverWorld, final int xCoord, final int yCoord, final int zCoord) {
        if (serverWorld.getBlockId(xCoord, yCoord, zCoord) != this.blockID) {
            return;
        }
        if (this.blockMaterial == Material.LAVA) {
            int n = 0;
            if (n != 0 || serverWorld.getBlockMaterial(xCoord, yCoord, zCoord - 1) == Material.WATER) {
                n = 1;
            }
            if (n != 0 || serverWorld.getBlockMaterial(xCoord, yCoord, zCoord + 1) == Material.WATER) {
                n = 1;
            }
            if (n != 0 || serverWorld.getBlockMaterial(xCoord - 1, yCoord, zCoord) == Material.WATER) {
                n = 1;
            }
            if (n != 0 || serverWorld.getBlockMaterial(xCoord + 1, yCoord, zCoord) == Material.WATER) {
                n = 1;
            }
            if (n != 0 || serverWorld.getBlockMaterial(xCoord, yCoord + 1, zCoord) == Material.WATER) {
                n = 1;
            }
            if (n != 0) {
                final int blockMetadata = serverWorld.getBlockMetadata(xCoord, yCoord, zCoord);
                if (blockMetadata == 0) {
                    serverWorld.setBlockWithNotify(xCoord, yCoord, zCoord, Block.obsidian.blockID);
                } else if (blockMetadata <= 4) {
                    serverWorld.setBlockWithNotify(xCoord, yCoord, zCoord, Block.cobblestone.blockID);
                }
    ...
    protected void triggerLavaMixEffects(final ServerWorld serverWorld, final int xCoord, final int yCoord,
            final int zCoord) {
        serverWorld.playSoundEffect((xCoord + 0.5f), (yCoord + 0.5f), (zCoord + 0.5f), "random.fizz", 0.5f,
                2.6f + (serverWorld.rand.nextFloat() - serverWorld.rand.nextFloat()) * 0.8f);
        for (int i = 0; i < 8; ++i) {
            serverWorld.spawnParticle("largesmoke", xCoord + random(), yCoord + 1.2, zCoord + random(), 0.0, 0.0, 0.0);
        }
    }
    ...
    public static double getFlowDirection(final IBlockAccess blockAccess, final int xCoord, final int yCoord,
            final int zCoord, final Material material) {
        Vec3 vec3D = null;
