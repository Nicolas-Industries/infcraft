...
public class EntityList {
    ...
    public static Entity createEntityInWorld(final String string, final ServerWorld serverWorld) {
        Entity entity = null;
        try {
            final Class clazz = (Class) EntityList.stringToClassMapping.get(string);
            if (clazz != null) {
                entity = (Entity) clazz.getConstructor(new Class[]{ServerWorld.class}).newInstance(new Object[]{serverWorld});
            }
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    public static Entity createEntityFromNBT(final NBTTagCompound nbt, final ServerWorld serverWorld) {
        Entity entity = null;
        try {
