...
package net.infcraft.core.world.chunk;

import net.infcraft.core.enums.EnumSkyBlock;
import net.infcraft.core.blocks.Block;
import net.infcraft.core.blocks.ContainerBlock;
...
public class Chunk {
    ...
    public ServerWorld serverWorldObj;
    ...
    public Chunk(final ServerWorld serverWorld, final int xCoord, final int zCoord) {
        ...
        this.serverWorldObj = serverWorld;
    ...
    public Chunk(final ServerWorld serverWorld, final byte[] byteArray, final int xCoord, final int zCoord) {
        this(serverWorld, xCoord, zCoord);
    ...
    private void checkSkylightNeighborHeight(final int integer1, final int integer2, final int integer3) {
        final int heightValue = this.serverWorldObj.getHeightValue(integer1, integer2);
        if (heightValue > integer3) {
            this.serverWorldObj.scheduleLightingUpdate(EnumSkyBlock.Sky, integer1, integer3, integer2, integer1, heightValue, integer2);
        } else if (heightValue < integer3) {
            this.serverWorldObj.scheduleLightingUpdate(EnumSkyBlock.Sky, integer1, heightValue, integer2, integer1, integer3, integer2);
        }
        this.isModified = true;
    }
    ...
    private void relightBlock(final int xCoord, final int yCoord, final int zCoord) {
        ...
        this.serverWorldObj.markBlocksDirtyVertical(xCoord, zCoord, n2, n);
        ...
        if (n2 < n) {
            ...
            this.serverWorldObj.scheduleLightingUpdate(EnumSkyBlock.Sky, n3, n, i, n3, n2, i);
            for (int j = n; j < n2; ++j) {
        ...
        if (n2 != integer6) {
            this.serverWorldObj.scheduleLightingUpdate(EnumSkyBlock.Sky, n3 - 1, n2, i - 1, n3 + 1, integer6, i + 1);
        }
        this.isModified = true;
    }
    ...
    public boolean setBlockIDWithMetadata(final int integer1, final int integer2, final int integer3, final int integer4, final int integer5) {
        ...
        if (n2 != 0) {
            Block.blocksList[n2].onBlockRemoval(this.serverWorldObj, xCoord, integer2, zCoord);
        }
        ...
        this.serverWorldObj.scheduleLightingUpdate(EnumSkyBlock.Sky, xCoord, integer2, zCoord, xCoord, integer2, zCoord);
        this.serverWorldObj.scheduleLightingUpdate(EnumSkyBlock.Block, xCoord, integer2, zCoord, xCoord, integer2, zCoord);
        this.propagateSkylightOcclusion(integer1, integer3);
        if (integer4 != 0) {
            Block.blocksList[integer4].onBlockAdded(this.serverWorldObj, xCoord, integer2, zCoord);
        }
    ...
    public TileEntity getChunkBlockTileEntity(final int xCoord, final int yCoord, final int zCoord) {
        ...
        if (tileEntity == null) {
            ((ContainerBlock) Block.blocksList[this.getBlockID(xCoord, yCoord, zCoord)]).onBlockAdded(this.serverWorldObj, this.xPosition * 16 + xCoord, yCoord, this.zPosition * 16 + zCoord);
    ...
    public void setChunkBlockTileEntity(final int xCoord, final int yCoord, final int zCoord, final TileEntity tileEntity) {
        final int n = xCoord + yCoord * 1024 + zCoord * 1024 * 1024;
        tileEntity.serverWorldObj = this.serverWorldObj;
        ...
        if (this.isChunkLoaded) {
            if (this.chunkTileEntityMap.get(n) != null) {
                this.serverWorldObj.loadedTileEntityList.remove(this.chunkTileEntityMap.get(n));
            }
            this.serverWorldObj.loadedTileEntityList.add(tileEntity);
        }
    ...
    public void removeChunkBlockTileEntity(final int integer1, final int integer2, final int integer3) {
        ...
        if (this.isChunkLoaded) {
            this.serverWorldObj.loadedTileEntityList.remove(this.chunkTileEntityMap.remove(n));
        }
    }

    public void onChunkLoad() {
        this.isChunkLoaded = true;
        this.serverWorldObj.loadedTileEntityList.addAll(this.chunkTileEntityMap.values());
        for (int i = 0; i < this.entities.length; ++i) {
            this.serverWorldObj.addLoadedEntities(this.entities[i]);
        }
    }

    public void onChunkUnload() {
        this.isChunkLoaded = false;
        this.serverWorldObj.loadedTileEntityList.removeAll(this.chunkTileEntityMap.values());
        for (int i = 0; i < this.entities.length; ++i) {
            this.serverWorldObj.unloadEntities(this.entities[i]);
        }
    }
    ...
    public void getEntitiesWithinAABBForEntity(final Entity entity, final AABB aabb, final List list) {
        int floor_double = Mth.floor_double((aabb.minY - 2.0) / 16.0);
        int floor_double2 = Mth.floor_double((aabb.maxY + 2.0) / 16.0);
        if (floor_double < 0) {
            floor_double = 0;
        }
    ...
    public boolean needsSaving(final boolean boolean1) {
        return !this.neverSave && ((this.hasEntities && this.serverWorldObj.getWorldTime != this.lastSaveTime) || this.isModified);
    }
}
