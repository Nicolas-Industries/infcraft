...
import net.infcraft.core.util.CompressedStreamTools;
import net.infcraft.core.blocks.Block;
import net.infcraft.core.blocks.LiquidBlock;
...
import net.infcraft.core.pathfinder.PathEntity;
import net.infcraft.core.pathfinder.Pathfinder;
...
import net.infcraft.core.world.*;
import net.infcraft.core.world.chunk.Chunk;
import net.infcraft.core.world.chunk.ChunkCache;
import net.infcraft.core.world.chunk.ChunkProviderGenerate;
import net.infcraft.core.world.chunk.ChunkProviderLoadOrGenerate;
import net.infcraft.core.world.chunk.storage.ChunkLoader;
import net.infcraft.shared.network.packets.PacketEntityDespawn;
import net.infcraft.shared.network.packets.PacketSetSlot;
...
public class ServerWorld implements World {
    ...
    static {
        ServerWorld.lightBrightnessTable = new float[16];
        final float n = 0.05f;
        for (int i = 0; i <= 15; ++i) {
            final float n2 = 1.0f - i / 15.0f;
            ServerWorld.lightBrightnessTable[i] = (1.0f - n2) / (n2 * 3.0f + 1.0f) * (1.0f - n) + n;
        }
    }

    public ServerWorld(final File file, final String string, Server server) {
        this(file, string, new Random().nextLong(), server);
    }

    public ServerWorld(final File file, final String string) {
        this(file, string, new Random().nextLong(), null);
    }

    public ServerWorld(final File file, final String string, final long long3, Server server) {
    ...
    public float getLightBrightness(final int nya1, final int nya2, final int nya3) {
        return ServerWorld.lightBrightnessTable[this.getBlockLightValue(nya1, nya2, nya3)];
    }
    ...
    public void playSound(final Entity entity, final String soundName, final float volume, final float pitch) {
        ...
        // Send packet to all players EXCEPT the source entity
        // This allows the client to play the sound locally (instant feedback)
        // while other players receive the packet (networked sound)
        net.infcraft.shared.network.packets.PacketPlaySound packet = new net.infcraft.shared.network.packets.PacketPlaySound(
                soundName, entity.posX, entity.posY, entity.posZ, volume, pitch);

        if (entity instanceof net.infcraft.core.entity.EntityPlayer) {
            this.server.getNetworkSystem().sendPacketToAllExcept(packet,
                    (net.infcraft.core.entity.EntityPlayer) entity);
        } else {
            // If source is not a player, broadcast to everyone
            this.server.getNetworkSystem().broadcastPacketToAll(packet);
        }
    }
    ...
    public void playSoundEffect(final double xCoord, final double yCoord, final double zCoord, final String soundName,
        ...
        net.infcraft.shared.network.packets.PacketPlaySound packet = new net.infcraft.shared.network.packets.PacketPlaySound(
                soundName, xCoord, yCoord, zCoord, volume, pitch);
        if (this.server != null && this.server.getNetworkSystem() != null) {
            this.server.getNetworkSystem().broadcastPacketToAll(packet);
        }
    }
    ...
    public void entityJoinedWorld(final Entity entity) {
        ...
        System.out.println(
                "SERVER: entityJoinedWorld - entity " + entity.getClass().getSimpleName() + " ID=" + entity.entityId +
                        " at chunk (" + floor_double + "," + floor_double2 + ")");
        System.out.println("SERVER: worldAccesses.size() = " + this.worldAccesses.size());
        if (this.chunkExists(floor_double, floor_double2)) {
            ...
            System.out.println("SERVER: Notifying " + this.worldAccesses.size() + " world listeners");
            for (int i = 0; i < this.worldAccesses.size(); ++i) {
                System.out.println("SERVER: Calling obtainEntitySkin on listener " + i + ": "
                        + this.worldAccesses.get(i).getClass().getSimpleName());
                ((IWorldAccess) this.worldAccesses.get(i)).obtainEntitySkin(entity);
            }

            // For non-player entities, broadcast spawn packets to clients
            if (!(entity instanceof net.infcraft.core.entity.EntityPlayer) &&
                    this.server != null && this.server.getNetworkSystem() != null) {
                try {
                    // For now, we'll handle item entities specifically
                    if (entity instanceof net.infcraft.core.entity.EntityItem) {
                        net.infcraft.core.entity.EntityItem itemEntity = (net.infcraft.core.entity.EntityItem) entity;
                        net.infcraft.shared.network.packets.PacketSpawnItem spawnPacket = new net.infcraft.shared.network.packets.PacketSpawnItem(
                                entity.entityId,
                                entity.posX, entity.posY, entity.posZ,
                                entity.motionX, entity.motionY, entity.motionZ,
                                itemEntity.item);

                        // Broadcast to all players and track that they know about this entity
                        this.server.getNetworkSystem().broadcastPacketToAll(spawnPacket);

                        // Update entity tracking for all players
                        for (String username : this.server.getNetworkSystem().getPlayerUsernames()) {
                            this.server.getNetworkSystem().addEntityToPlayer(username, entity.entityId);
                        }
                    }
                    // Add more entity types as needed
                } catch (Exception e) {
                    System.err.println("Error sending entity spawn packet: " + e.getMessage());
                }
            }
        } else {
    ...
    public void setEntityDead(final Entity entity) {
        // Send despawn packet to all clients before removing the entity
        if (this.server != null && this.server.getNetworkSystem() != null) {
            try {
                PacketEntityDespawn despawnPacket = new PacketEntityDespawn(entity.entityId);
                this.server.getNetworkSystem().broadcastPacketToAll(despawnPacket);

                // Remove entity from all players' known entities
                for (String username : this.server.getNetworkSystem().getPlayerUsernames()) {
                    this.server.getNetworkSystem().removeEntityFromPlayer(username, entity.entityId);
                }
            } catch (Exception e) {
                System.err.println("Error sending entity despawn packet: " + e.getMessage());
            }
        }
    ...
    public void notifyEntityPickup(Entity collector, Entity collected) {
        for (int i = 0; i < this.worldAccesses.size(); ++i) {
            ((IWorldAccess) this.worldAccesses.get(i)).onEntityPickup(collector, collected);
        }

        // Send inventory update to client after item pickup
        if (collector instanceof EntityPlayer && server != null && server.getNetworkSystem() != null) {
            EntityPlayer player = (EntityPlayer) collector;

            // Find the slot that changed (we'll send the slot where the item was picked up)
            ...
            if (player.inventory.getCurrentItem() != null) {
                ...
                PacketSetSlot packet = new PacketSetSlot(0, slot, player.inventory.getCurrentItem());

                try {
                    server.getNetworkSystem().sendPacketToPlayer(player.getUsername(), packet);
                } catch (Exception e) {
                    System.err.println("Error sending inventory update after pickup: " + e.getMessage());
                }
            }
        }
    }
