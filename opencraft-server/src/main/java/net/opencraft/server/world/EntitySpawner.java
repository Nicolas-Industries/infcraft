...
public class EntitySpawner {
    ...
    public EntitySpawner(final int integer, final Class class2, final Class[] arr) {
        this.a = integer;
        this.b = class2;
        this.c = arr;
    }

    public void a(final ServerWorld fe) {
        if (fe.countEntities(this.b) < this.a) {
            for (int i = 0; i < 10; ++i) {
                //this.a(fe, 1, fe.player, null);
                // TODO: implement player selection for multiple players
                this.a(fe, 1, null, null);
            }
        }
    }

    protected ChunkPosition a(final ServerWorld fe, final int integer2, final int integer3) {
        return new ChunkPosition(integer2 + fe.rand.nextInt(256) - 128, fe.rand.nextInt(128), integer3 + fe.rand.nextInt(256) - 128);
    }

    private int a(final ServerWorld fe, final int integer, final Entity eq, final IProgressUpdate jd) {
        int n = 0;
...
//                    } else { TODO: re-implement for multiple players
...
//                        }
                    ...
                    EntityLiving entity;
                    try {
                        entity = (EntityLiving) this.c[nextInt].getConstructor(new Class[]{ServerWorld.class}).newInstance(new Object[]{fe});
                    } catch (Exception ex) {
                        ex.printStackTrace();
                        return n;
                    }
