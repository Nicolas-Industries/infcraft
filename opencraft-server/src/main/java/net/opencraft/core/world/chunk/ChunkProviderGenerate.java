...
import net.opencraft.core.world.IProgressUpdate;
import net.opencraft.core.world.gen.WorldGenDungeons;
import net.opencraft.core.world.gen.WorldGenMinable;
import net.opencraft.core.util.Mth;
import net.opencraft.core.world.IChunkProvider;
import net.opencraft.server.world.ServerWorld;
import net.opencraft.core.world.gen.*;
...
public class ChunkProviderGenerate implements IChunkProvider {
	...
	private ServerWorld serverWorldObj;
	...
	public ChunkProviderGenerate(final ServerWorld fe, final long long2) {
		this.field_914_i = new int[32][32];
		this.serverWorldObj = fe;
	...
	public Chunk provideChunk(final int integer1, final int integer2) {
		...
		final Chunk chunk = new Chunk(this.serverWorldObj, array, integer1, integer2);
	...
	private void field_902_u(final int integer1, final int integer2, final byte[] arr) {
		final int n = 8;
		this.rand.setSeed(this.serverWorldObj.n);
		...
		for (int i = integer1 - n; i <= integer1 + n; ++i) {
			for (int j = integer2 - n; j <= integer2 + n; ++j) {
				this.rand.setSeed(i * n2 + j * n3 ^ this.serverWorldObj.n);
	...
	public void populate(final IChunkProvider ch, final int integer2, final int integer3) {
		...
		if (this.rand.nextInt(1) == 0) {
			...
			new WorldGenMinable(Block.oreGold.blockID, 8).generate(this.serverWorldObj, this.rand, i, integer4, j);
		}
		if (this.rand.nextInt(4) == 0) {
			final int i = n + this.rand.nextInt(16);
		...
		if (this.rand.nextInt(2) == 0) {
			...
			final int n4 = n2 + this.rand.nextInt(16) + 8;
			new WorldGenFlowers(Block.plantRed.blockID).generate(this.serverWorldObj, this.rand, j, integer5, n4);
		}
		if (this.rand.nextInt(4) == 0) {
			final int j = n + this.rand.nextInt(16) + 8;
			final int integer5 = this.rand.nextInt(128);
			final int n4 = n2 + this.rand.nextInt(16) + 8;
			new WorldGenFlowers(Block.mushroomBrown.blockID).generate(this.serverWorldObj, this.rand, j, integer5, n4);
		}
		if (this.rand.nextInt(8) == 0) {
			final int j = n + this.rand.nextInt(16) + 8;
			final int integer5 = this.rand.nextInt(128);
			final int n4 = n2 + this.rand.nextInt(16) + 8;
			new WorldGenFlowers(Block.mushroomRed.blockID).generate(this.serverWorldObj, this.rand, j, integer5, n4);
		}
		for (int j = 0; j < 50; ++j) {
			final int integer5 = n + this.rand.nextInt(16) + 8;
