...
import net.opencraft.core.world.IChunkLoader;
...
public class ChunkProviderLoadOrGenerate implements IChunkProvider {
    ...
    public ChunkProviderLoadOrGenerate(final ServerWorld fe, final IChunkLoader bg, final IChunkProvider ch) {
    ...
    private Chunk func_542_c(final int integer1, final int integer2) {
        ...
        try {
            final Chunk loadChunk = this.chunkLoader.loadChunk(this.serverWorldObj, integer1, integer2);
            if (loadChunk != null) {
                loadChunk.lastSaveTime = this.serverWorldObj.getWorldTime;
            }
            return loadChunk;
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    private void saveExtraChunkData(final Chunk jw) {
        ...
        try {
            this.chunkLoader.saveExtraChunkData(this.serverWorldObj, jw);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
    ...
    private void saveChunk(final Chunk jw) {
        ...
        jw.lastSaveTime = this.serverWorldObj.getWorldTime;
        this.chunkLoader.saveChunk(this.serverWorldObj, jw);
    }
