...
public class ChunkCache implements IBlockAccess {
    ...
    public ChunkCache(final ServerWorld serverWorld, final int integer2, final int integer3, final int integer4, final int integer5, final int integer6, final int integer7) {
        ...
        for (int i = this.chunkX; i <= n; ++i) {
            for (int j = this.chunkZ; j <= n2; ++j) {
                this.chunkArray[i - this.chunkX][j - this.chunkZ] = serverWorld.getChunkFromChunkCoords(i, j);
            }
        }
    }
    ...
    public float getLightBrightness(final int nya1, final int nya2, final int nya3) {
        return ServerWorld.lightBrightnessTable[this.getLightValue(nya1, nya2, nya3)];
    }
    ...
    public int getLightValueExt(final int xCoord, final int yCoord, final int zCoord, final boolean boolean4) {
        ...
        if (yCoord >= 128) {
            int blockId = 15 - this.serverWorldObj.skylightSubtracted;
            if (blockId < 0) {
                blockId = 0;
            }
            return blockId;
        ...
        return this.chunkArray[blockId][lightValueExt].getBlockLightValue(xCoord & 0xF, yCoord, zCoord & 0xF, this.serverWorldObj.skylightSubtracted);
    }
