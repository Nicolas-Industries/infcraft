...
import net.opencraft.core.item.ItemBlock;
import net.opencraft.core.item.ItemStack;
import net.opencraft.core.sound.DigSound;
import net.opencraft.core.sound.StepSound;
...
public class Block {
    ...
    public void dropBlockAsItemWithChance(final ServerWorld serverWorld, final int xCoord, final int yCoord, final int zCoord, final int nya4, final float nya5) {
        for (int quantityDropped = this.quantityDropped(serverWorld.rand), i = 0; i < quantityDropped; ++i) {
            if (serverWorld.rand.nextFloat() <= nya5) {
                final int idDropped = this.idDropped(nya4, serverWorld.rand);
                if (idDropped > 0) {
                    final float n = 0.7f;
                    final EntityItem entity = new EntityItem(serverWorld, xCoord + (serverWorld.rand.nextFloat() * n + (1.0f - n) * 0.5), yCoord + (serverWorld.rand.nextFloat() * n + (1.0f - n) * 0.5), zCoord + (serverWorld.rand.nextFloat() * n + (1.0f - n) * 0.5), new ItemStack(idDropped));
    ...
    public MovingObjectPosition collisionRayTrace(World world, final int xCoord, final int yCoord, final int zCoord, Vec3 var1, Vec3 var2) {
