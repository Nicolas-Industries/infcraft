
package net.opencraft.core.blocks;

import net.opencraft.core.blocks.material.Material;
import net.opencraft.core.physics.AABB;
import net.opencraft.core.world.World;
import net.opencraft.server.world.ServerWorld;

import java.util.Random;

public class FlowerBlock extends Block {

    protected FlowerBlock(final int blockid, final int blockIndexInTexture) {
        super(blockid, Material.PLANTS);
        this.blockIndexInTexture = blockIndexInTexture;
        this.setTickOnLoad(true);
        final float n = 0.2f;
        this.setShape(0.5f - n, 0.0f, 0.5f - n, 0.5f + n, n * 3.0f, 0.5f + n);
    }

    @Override
    public boolean canPlaceBlockAt(final ServerWorld serverWorld, final int xCoord, final int yCoord, final int zCoord) {
        return this.canThisPlantGrowOnThisBlockID(serverWorld.getBlockId(xCoord, yCoord - 1, zCoord));
    }

    protected boolean canThisPlantGrowOnThisBlockID(final int blockid) {
        return blockid == Block.grass.blockID || blockid == Block.dirt.blockID || blockid == Block.tilledField.blockID;
    }

    @Override
    public void onNeighborBlockChange(final ServerWorld serverWorld, final int xCoord, final int yCoord, final int zCoord, final int nya4) {
        super.onNeighborBlockChange(serverWorld, xCoord, yCoord, zCoord, nya4);
        this.checkFlowerChange(serverWorld, xCoord, yCoord, zCoord);
    }

    @Override
    public void updateTick(final ServerWorld serverWorld, final int xCoord, final int yCoord, final int zCoord, final Random random) {
        this.checkFlowerChange(serverWorld, xCoord, yCoord, zCoord);
    }

    protected final void checkFlowerChange(final ServerWorld serverWorld, final int xCoord, final int yCoord, final int zCoord) {
        if (!this.canBlockStay(serverWorld, xCoord, yCoord, zCoord)) {
            this.dropBlockAsItem(serverWorld, xCoord, yCoord, zCoord, serverWorld.getBlockMetadata(xCoord, yCoord, zCoord));
            serverWorld.setBlockWithNotify(xCoord, yCoord, zCoord, 0);
        }
    }

    public boolean canBlockStay(final ServerWorld serverWorld, final int xCoord, final int yCoord, final int zCoord) {
        return (serverWorld.getBlockLightValue(xCoord, yCoord, zCoord) >= 8 || serverWorld.canBlockSeeTheSky(xCoord, yCoord, zCoord)) && this.canThisPlantGrowOnThisBlockID(serverWorld.getBlockId(xCoord, yCoord - 1, zCoord));
    }

    @Override
    public AABB getCollisionBoundingBoxFromPool(final World world, final int xCoord, final int yCoord, final int zCoord) {
        return null;
    }

    @Override
    public boolean isOpaqueCube() {
        return false;
    }

    @Override
    public boolean renderAsNormalBlock() {
        return false;
    }

    @Override
    public int getRenderType() {
        return 1;
    }
}
