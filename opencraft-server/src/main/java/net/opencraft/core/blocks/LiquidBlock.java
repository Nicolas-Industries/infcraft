...
public abstract class LiquidBlock extends Block {
    ...
    public void velocityToAddToEntity(final ServerWorld serverWorld, final int xCoord, final int yCoord, final int zCoord, final Entity entity, final Vec3 var1) {
        final Vec3 flowVector = this.getFlowVector(serverWorld, xCoord, yCoord, zCoord);
        var1.x += flowVector.x;
        var1.y += flowVector.y;
        var1.z += flowVector.z;
    }
    ...
    public void randomDisplayTick(final ServerWorld serverWorld, final int xCoord, final int yCoord, final int zCoord, final Random random) {
        if (this.blockMaterial == Material.WATER && random.nextInt(64) == 0) {
            ...
            if (blockMetadata > 0 && blockMetadata < 8) {
                serverWorld.playSoundEffect((xCoord + 0.5f), (yCoord + 0.5f), (zCoord + 0.5f), "liquid.water", random.nextFloat() * 0.25f + 0.75f, random.nextFloat() * 1.0f + 0.5f);
            }
        }
        if (this.blockMaterial == Material.LAVA && serverWorld.getBlockMaterial(xCoord, yCoord + 1, zCoord) == Material.AIR && !serverWorld.isBlockNormalCube(xCoord, yCoord + 1, zCoord) && random.nextInt(100) == 0) {
            serverWorld.spawnParticle("lava", (xCoord + random.nextFloat()), yCoord + this.maxY, (zCoord + random.nextFloat()), 0.0, 0.0, 0.0);
        }
    }
    ...
    protected void triggerLavaMixEffects(final ServerWorld serverWorld, final int xCoord, final int yCoord, final int zCoord) {
        serverWorld.playSoundEffect((xCoord + 0.5f), (yCoord + 0.5f), (zCoord + 0.5f), "random.fizz", 0.5f, 2.6f + (serverWorld.rand.nextFloat() - serverWorld.rand.nextFloat()) * 0.8f);
        for (int i = 0; i < 8; ++i) {
